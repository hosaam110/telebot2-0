...
def ask_student_status(message):
    ...
    bot.send_message(message.chat.id, questions[current_question]["text"], reply_markup=keyboard)

def ask_arabic_name(message):
    bot.send_message(message.chat.id, questions[current_question]["text"])

def ask_english_name(message):
    bot.send_message(message.chat.id, questions[current_question]["text"])

def ask_date_of_birth(message):
    bot.send_message(message.chat.id, questions[current_question]["text"])
...
def ask_turkish_phone(message):
    bot.send_message(message.chat.id, questions[current_question]["text"])

def ask_whatsapp_number(message):
    bot.send_message(message.chat.id, questions[current_question]["text"])

def ask_residence_location(message):
...
def ask_academic_level(message):
    keyboard = create_academic_level_keyboard()
    bot.send_message(message.chat.id, questions[current_question]["text"], reply_markup=keyboard)

def ask_orange_belt(message):
    bot.send_message(message.chat.id, questions[current_question]["text"])

def ask_id_front(message):
    bot.send_message(message.chat.id, questions[current_question]["text"])

def ask_id_back(message):
    bot.send_message(message.chat.id, questions[current_question]["text"])
...
def handle_messages(message):
    ...
    if current_question in [0, 4, 7, 11, 15]:
        if message.content_type == 'text':
            bot.send_message(message.chat.id, "يرجى الاختيار من الخيارات المتاحة.")
            return

    # elif current_question == 1:
    #     # Validate Arabic name
    #     if not check_arabic_name(user_input):
    #         bot.send_message(message.chat.id, "الرجاء إدخال اسم عربي صحيح مكون من أربعة أسماء.")
    #         return
    #     user_data[current_question] = user_input
    # elif current_question == 2:
    #     # Validate English name
    #     if not check_english_name(user_input):
    #         bot.send_message(message.chat.id, "الرجاء إدخال اسم إنجليزي صحيح مكون من أربعة أسماء.")
    #         return
    #     user_data[current_question] = user_input
    # elif current_question == 3:
    #     # Validate date of birth
    #     if not validate_date(user_input):
    #         bot.send_message(message.chat.id, "الرجاء إدخال تاريخ الميلاد بالصيغة الصحيحة YYYY-MM-DD.")
    #         return
    #     user_data[current_question] = user_input
    # elif current_question == 5:
    #     # Validate Turkish phone number
    #     if not check_turkish_phone_number(user_input):
    #         bot.send_message(message.chat.id, "الرجاء إدخال رقم هاتف تركي صحيح مكون من أرقام فقط ويبدأ بـ 05.")
    #         return
    #     user_data[current_question] = user_input
    # elif current_question == 6:
    #     # Validate WhatsApp number
    #     if not check_whatsapp_number(user_input):
    #         bot.send_message(message.chat.id, "الرجاء إدخال رقم واتساب صحيح مكون من أرقام فقط ويبدأ بـ ...+ مع مفتاح الخط الدولي.")
    #         return
    #     user_data[current_question] = user_input
    ...
    #         bot.send_message(message.chat.id, "الرجاء إدخال اسم الجامعة بالإنجليزي.")
    #         return
    #     user_data[current_question] = user_input
    ...
    #     user_data[current_question] = user_input
        ...
        if message.content_type == 'document' and message.document.mime_type == 'application/pdf':
            save_file_to_drive(message, current_question)
        elif message.content_type == 'photo' and message.photo[0].file_size < 5242880:
            save_file_to_drive(message, current_question)
        else:
            bot.send_message(message.chat.id, "يرجى إرسال ملف PDF أو صورة JPG صالحة.")
            return  # Do not increase current_question
    
    current_question += 1

    if current_question < len(questions):
        bot.send_message(message.chat.id, questions[current_question]["text"], reply_markup=create_keyboard_for_question(current_question))
    else:
        save_data(message)  # استدعاء دالة حفظ البيانات بعد جمعها من المستخدم
...
# اسم ملف الاعتماد
creds = ServiceAccountCredentials.from_json_keyfile_name('student-information-408604-1a1b268e6dff.json', scope)

# اتصال بواجهة برمجة التطبيقات
client = gspread.authorize(creds)

# معرّف ورقة البيانات
spreadsheet_id = '1gix-xC50yCyx7KNH67EVgWcYo9TnbcpRi2hToqXyC8A'
...
def save_data(message):
    constant_prefix = "2024"  # تعريف الجزء الثابت هنا

    # جمع البيانات التي ترغب في حفظها
    data_to_save = [
        generate_membership_number(),  # رقم العضوية
    ...
    if success:
        bot.send_message(message.chat.id, f"تم حفظ البيانات برقم العضوية: {data_to_save[0]}")
    else:
        bot.send_message(message, f"حدث خطأ أثناء محاولة حفظ البيانات في ورقة البيانات: {error_message}")# دالة توليد رقم عضوية متسلسل
    
def generate_membership_number():
    # تعيين الرقم الثابت للعام 2024
    constant_prefix = "2024"
    # جلب عدد السجلات الموجودة حاليًا في ورقة البيانات لتوليد رقم عضوية جديد
    try:
        sheet = client.open_by_key(spreadsheet_id).sheet1
        records_count = len(sheet.get_all_values())  # عدد الصفوف الموجودة في ورقة البيانات حتى الآن
    except Exception as e:
        print(f"Error fetching data from Google Spreadsheet: {str(e)}")
        return None
    # توليد رقم عضوية جديد باستخدام عدد السجلات الحالية + 1
    new_membership_number = f"{constant_prefix}{str(records_count + 1).zfill(4)}"  # تكوين رقم العضوية الجديد
